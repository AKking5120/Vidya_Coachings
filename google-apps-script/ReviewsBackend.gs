...
function setupSheet() {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(['Name', 'Role', 'Rating', 'Review', 'Date', 'Approved', 'IP Address']);
    return;
  }
  var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  if (headers.indexOf('IP Address') === -1) {
    sheet.getRange(1, sheet.getLastColumn() + 1).setValue('IP Address');
  }
}
...
function doPost(e) {
  ...
  sheet.appendRow([name, role, rating, text, new Date(), 'pending', ip]);
